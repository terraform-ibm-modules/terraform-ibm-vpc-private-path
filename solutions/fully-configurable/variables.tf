...
variable "private_path_tags" {
  ...
  nullable    = false
  validation {
    condition     = alltrue([for tag in var.private_path_tags : can(regex("^[A-Za-z0-9 _\\-.:]{1,128}$", tag))])
...
variable "private_path_access_tags" {
  ...
  validation {
    condition = alltrue([
      for tag in var.private_path_access_tags : can(regex("[\\w\\-_\\.]+:[\\w\\-_\\.]+", tag)) && length(tag) <= 128
    ])
